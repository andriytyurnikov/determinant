...
pub const Cpu = struct {
    ...
    fn readByte(self: *const Cpu, addr: u32) !u8 {
        if (addr >= MEMORY_SIZE) return error.AddressOutOfBounds;
        return self.memory[addr];
    }

    fn readHalfword(self: *const Cpu, addr: u32) !u16 {
    ...
    fn readWord(self: *const Cpu, addr: u32) !u32 {
    ...
    fn writeByte(self: *Cpu, addr: u32, value: u8) !void {
        if (addr >= MEMORY_SIZE) return error.AddressOutOfBounds;
        self.memory[addr] = value;
    }

    fn writeHalfword(self: *Cpu, addr: u32, value: u16) !void {
        if (addr % 2 != 0) return error.MisalignedAccess;
        if (addr > MEMORY_SIZE - 2) return error.AddressOutOfBounds;
        const a: usize = addr;
    ...
    fn writeWord(self: *Cpu, addr: u32, value: u32) !void {
        if (addr % 4 != 0) return error.MisalignedAccess;
        if (addr > MEMORY_SIZE - 4) return error.AddressOutOfBounds;
        const a: usize = addr;
