...
pub fn CpuType(comptime memory_size: u32) type {
    ...
    return struct {
        const Self = @This();
        pub const MEMORY_SIZE: u32 = memory_size;
        ...
        pub fn step(self: *Self) !StepResult {
            ...
            const inst = try lut_decoder.decodeInstruction(raw);
        ...
        fn executeI(self: *Self, op: rv32i.Opcode, inst: instructions.Instruction, rs1_val: u32, rs2_val: u32, inst_size: u32, next_pc: *u32) !StepResult {
            const imm_u: u32 = inst.immUnsigned();
            switch (op) {
                // R-type ALU
        ...
        fn executeM(self: *Self, op: rv32m.Opcode, rd_reg: u5, rs1_val: u32, rs2_val: u32) void {
        ...
        fn executeA(self: *Self, op: rv32a.Opcode, rd_reg: u5, rs1_val: u32, rs2_val: u32) !void {
            const addr = rs1_val;
            switch (op) {
                .LR_W => {
        ...
        fn executeZba(self: *Self, op: zba.Opcode, rd_reg: u5, rs1_val: u32, rs2_val: u32) void {
            self.writeReg(rd_reg, zba.execute(op, rs1_val, rs2_val));
        }

        // --- Zbb helpers ---

        fn executeZbb(self: *Self, op: zbb.Opcode, rd_reg: u5, rs1_val: u32, rs2_val: u32, imm: u32) void {
        ...
        fn executeZbs(self: *Self, op: zbs.Opcode, rd_reg: u5, rs1_val: u32, rs2_val: u32, imm: u32) void {
            const src2: u32 = if (op.format() == .R) rs2_val else imm;
        ...
        fn executeCsr(self: *Self, op: zicsr.Opcode, rd_reg: u5, rs1_field: u5, rs1_val: u32, imm: i32) !void {
            // Truncate sign-extended immediate to recover 12-bit CSR address (see Instruction.csrAddr).
...
test "CpuType: custom memory size" {
    ...
    var c = SmallCpu.init();
    try std.testing.expectEqual(@as(u32, 4096), SmallCpu.MEMORY_SIZE);
    c.writeReg(1, 42);
