...
test "step: MUL basic" {
    ...
    cpu.writeReg(1, 7);
    cpu.writeReg(2, 6);
    // MUL x3, x1, x2: funct3=000, funct7=0000001
...
test "step: MULH signed positive" {
    var cpu = Cpu.init();
    // 0x10000 * 0x10000 = 0x1_0000_0000 → upper 32 bits = 1
...
test "step: MULH large negative" {
    var cpu = Cpu.init();
    // -1 * 1 = -1 → upper 32 bits = 0xFFFFFFFF
    cpu.writeReg(1, 0xFFFFFFFF); // -1
    cpu.writeReg(2, 1);
    loadInst(&cpu, encodeR(0b001, 0b0000001, 3, 1, 2));
    _ = try cpu.step();
...
test "step: MULHSU signed*unsigned" {
    var cpu = Cpu.init();
    // -1 (signed) * 2 (unsigned) = -2 → upper 32 bits = 0xFFFFFFFF
    cpu.writeReg(1, 0xFFFFFFFF); // -1 signed
    cpu.writeReg(2, 2);
...
test "step: DIV signed negative" {
    var cpu = Cpu.init();
    cpu.writeReg(1, @as(u32, @bitCast(@as(i32, -20)))); // -20
    cpu.writeReg(2, 6);
    loadInst(&cpu, encodeR(0b100, 0b0000001, 3, 1, 2));
    _ = try cpu.step();
    // -20 / 6 = -3 (truncate toward zero)
